...
const int QCefSetting::devToolWidth()
{
    CCefSetting::initializeInstance();
    return CCefSetting::devToolWidth;
}

const int QCefSetting::devToolHeight()
{
    CCefSetting::initializeInstance();
    return CCefSetting::devToolHeight;
}

/////////////////////////////////////////////
// 下面这两个属性需要通过command line传递给renderer进程，暂时放在这里
static int g_logLevel = 0;
void QCefSetting::setLogLevel(int logLevel)
{
    g_logLevel = logLevel;
}

int QCefSetting::logLevel()
{
    return g_logLevel;
}
